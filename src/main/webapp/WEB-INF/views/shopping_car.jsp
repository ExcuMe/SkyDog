	...
	<script type="text/javascript">
		...
		function updateShoppingCars() {
			...
			for(var i=0;i<allShoppingCars.length;i++){
				...
				html  += '<tr>'+
						...
						'</div>'+
						'</td >'+
						'<td style="color:gold">'+product.name+'</td>'+
						'<td style="color:gold">'+product.price+'</td>'+
						'<td style="color:gold">'+allShoppingCars[i].counts+'</td>'+
                    '<td >' +
                    '<button class="btn btn-primary btn-sm" onclick="deleteShoppingCar('+allShoppingCars[i].productId+')">删除</button>'+
					'</td>'+
						'</tr>';
			}
		...
		function buyConfirm(productsId,productsCounts) {
			...
			html +='<th>总金额：</th>'+
					...
					'<button class="btn  col-sm-4 col-md-4 col-lg-4" ' +
				'style="background-color :  #FF9800"  ' +
				'onclick="addToShoppingRecordsPre(['+productsId+'],['+productsCounts+'])">确认购买</button>'+
					'</div>'+
					'</div>';
			layer.open({
				type:1,
